...
def train_dqn_metrics(use_rnd=False):
    ...
    for episode in range(1000):
        ...
        sys.stdout.write(f"\repisode {episode}")
        obs, _ = env.reset()
        episode_return = 0
        done = False
        while not done:
            ...
            if hasattr(agent, "last_losses") and agent.last_losses is not None:
                metrics["extrinsic_loss"].append(
                    agent.last_losses.get("extrinsic", 0.0)
                )
                metrics["intrinsic_loss"].append(
                    agent.last_losses.get("intrinsic", 0.0)
                )
    ...
    for use_rnd, key in [(True, "rnd"), (False, "no_rnd")]:
        ...
        for k in keys:
            ...
            agg[k] = {
                ...
                "sem": (
                    np.std(arr, axis=0) / np.sqrt(n_seeds)
                    if n_seeds > 1
