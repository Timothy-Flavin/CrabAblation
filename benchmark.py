...
try:
    ...
    HAS_PSUTIL = False
    print(
        "Warning: psutil not installed. RAM prediction disabled. Run 'pip install psutil' to enable."
    )
...
def get_args():
    ...
    parser.set_defaults(
        rnd_burn_in=0, learning_starts=0, max_wall_time=15.0, skip_best_params=True
    )
        ...
        for dev in devices:
            ...
            for num_envs in num_envs_list:
                ...
                if HAS_PSUTIL and len(ram_measurements) >= 2:
                    ...
                    safe_limit = (
                        sys_mem.total * 0.85
                    )  # Cut off if approaching 85% of total system memory

                    if predicted_ram > safe_limit:
                        print(
                            f"⚠️ Skipping {num_envs}+ envs: Predicted RAM usage ({predicted_ram / 1024**3:.2f} GB) exceeds safe threshold."
                        )
                ...
                try:
                    ...
                    print(
                        f"Failed on ablation={ablation}, envs={num_envs}, dev={dev}. Skipping."
                    )
                    traceback.print_exc()
...
def benchmark_updates(
    ...
    if args.algo in ["sac", "dqn"]:
        ...
        t_end = time.time()
        print(
            f"Offline RL Update Benchmark for {args.algo.upper()} complete. Real Training Time: {(t_end - t_start):.3f}s."
        )
        return
